...
void setup() {
  ...
  Serial.println(F("Random OK"));

  #ifdef SAVE_GAME
  initEEPROM();
  Serial.println(F("initEEPROM OK"));
  #endif
...
void displayEndOfGame(bool playerDead) {
  ...
  if (playerDead) {
    ...
    #ifdef ESP8266
    EEPROM.write(EEPROM_SEQ_START, ALTERNATIVE_ENDING_PREFIX);
    EEPROM.write(EEPROM_SEQ_START + 1, ALTERNATIVE_ENDING_SEQ);
    EEPROM.commit();
    #else
